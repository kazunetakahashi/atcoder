...
public:
  ...
  void flush()
  {
    vector<int> DAoki(N), DTakahashi(N);
    DAoki[aoki] = 0;
    dfs(aoki, DAoki);
    DTakahashi[takahashi] = 0;
    dfs(takahashi, DTakahashi);
    int ans{0};
    for (auto i{0}; i < N; ++i)
    {
      if (DAoki[i] >= DTakahashi[i])
      {
        ch_max(ans, DAoki[i]);
      }
    }
...
private:
  void dfs(int src, vector<int> &D, int parent = -1)
  {
    for (auto e : V[src])
    {
      if (e.dst != parent)
