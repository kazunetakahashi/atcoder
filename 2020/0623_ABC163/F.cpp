...
class Solve
{
  ...
  vector<stack<SubTree>> colorToStack;
  vector<vector<ll>> colorToComponents;
  vector<ll> colorToCount;

public:
  Solve(ll N) : N{N}, toColor(N), id{0}, subTreeSize(N), colorToStack(N), colorToComponents(N), colorToCount(N, 0)
  {
    for (auto i{0}; i < N; ++i)
    {
      cin >> toColor[i];
      toColor[i]--;
      colorToCount[toColor[i]]++;
    }
    V = ReadTree(N);
  }
...
private:
  void dfs(ll src = 0, ll parent = -1)
  {
    ll src_id{id++};
    auto &stack{colorToStack[toColor[src]]};
    for (auto const &e : V[src])
    {
      ...
      dfs(e.dst, src);
      auto cutTreeSize{subTreeSize[e.dst]};
      while (!stack.empty() && stack.top().id > src_id)
      {
