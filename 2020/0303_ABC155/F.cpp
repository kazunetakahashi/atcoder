...
public:
  Solve(int N, int M) : N{N}, M{M}, bombs(N), flags(N + 1), V(N + 1), table(M, false), visited(N + 1, false)
  {
    for (auto i{0}; i < N; ++i)
    {
      cin >> get<0>(bombs[i]) >> get<1>(bombs[i]);
    }
    sort(bombs.begin(), bombs.end());
    flags[0] = get<1>(bombs[0]);
    for (auto i{1}; i < N; ++i)
    {
      flags[i] = get<1>(bombs[i]) ^ get<1>(bombs[i - 1]);
    }
    flags[N] = get<1>(bombs[N - 1]);
    for (auto i{0}; i < M; ++i)
    {
      int l, r;
      cin >> l >> r;
      int v{static_cast<int>(lower_bound(bombs.begin(), bombs.end(), Bomb{l, -1}) - bombs.begin())};
      int w{static_cast<int>(lower_bound(bombs.begin(), bombs.end(), Bomb{r, 2}) - bombs.begin())};
      Edge edge{v, w, i};
      edge.added_edge(V);
      edge.added_rev(V);
    }
