...
public:
  Solve(int N, int Q) : N{N}, Q{Q}, A(N), B(N), C(Q), D(Q), rate(M), tree{RangeMinQuery<int>(M)}
  {
    for (auto i{0}; i < N; ++i)
    {
      cin >> A[i] >> B[i];
    ...
    for (auto i{0}; i < M; ++i)
    {
      if (auto it{rate[i].begin()}; it != rate[i].end())
      {
        tree.update(i, *it);
      }
    }
...
private:
  int transfer(int rating, int src, int dst)
  {
    rate[src].erase(rate[src].find(rating));
    rate[dst].insert(rating);
    if (auto it{rate[src].begin()}; it != rate[src].end())
    {
      tree.update(src, *it);
    }
    else
    {
      tree.update(src, Infty<int>());
    }
    tree.update(dst, *rate[dst].begin());
    return tree.query(0, M);
  }
};
