...
int main()
{
  int N, Q;
  cin >> N >> Q;
  vector<int> to_rating(N);
  vector<int> to_garden(N);
  for (auto i{0}; i < N; ++i)
  {
    cin >> to_rating[i] >> to_garden[i];
    to_garden[i]--;
  }
  vector<multiset<int, greater<int>>> infants(C);
  for (auto i{0}; i < N; ++i)
  {
    infants[to_garden[i]].insert(to_rating[i]);
  }
  ...
  for (auto i{0}; i < C; ++i)
  {
    if (!infants[i].empty())
    {
      maximums.insert(*infants[i].begin());
    }
  }
  for (auto q{0}; q < Q; ++q)
  {
    ...
    infants[to_garden[id]].insert(to_rating[id]);
    maximums.insert(*infants[to_garden[id]].begin());
    cout << *maximums.begin() << endl;
  }
}
